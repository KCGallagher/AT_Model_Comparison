...
class ODEModel():
        ...
        while (currInterval[1] <= t_end) and (currCycleId < nCycles) and (not progressed):
            ...
            if atMethod == "Zhang":
                if self.resultsDf.TumourSize.iat[-1] < (
                        atThreshold * refSize):  # Withdraw treatment below a certain size
                    dose = 0
                    ...
                    dose = dose
            elif atMethod == "Threshold":  # No overshooting, single threshold
                if self.resultsDf.TumourSize.iat[-1] < (
                        atThreshold * refSize):  # Withdraw treatment below a certain size
                    dose = 0
                else:  # If size remains within a window of +- atThreshold, keep the same dose
            ...
            # Update interval
            # noiseStep = np.random.uniform(-noiseFrac, noiseFrac) * intervalLength
            noiseStep = np.random.exponential(scale=noiseFrac) * intervalLength
            # noiseStep = np.random.normal(scale=noiseFrac) * intervalLength
            modIntervalLength = (intervalLength + max(int(noiseStep), 0)) \
                if useRandomInterval else intervalLength

            currInterval = [currInterval[1], currInterval[1] + modIntervalLength]
            # print(currInterval, modIntervalLength, dose)
            # print(currInterval)
            currCycleId += 1
    ...
    # =========================================================================================
    # As well as Trimming, normalise data and remove post-progression records
    def CleanData(self, max_tumour_size=1.2):
        self.Trim()
        self.resultsDf[['S', 'R', 'TumourSize']] = (self.resultsDf[['S', 'R', 'TumourSize']] 
                                                    / self.resultsDf.TumourSize.iloc[0])
        self.resultsDf = self.resultsDf.loc[self.resultsDf.TumourSize <= max_tumour_size]
    ...
    # =========================================================================================
    # Function to plot the model predictions
    def CalcTtp(self, method='ROT', threshold = 0.5, updating_interval=1):
        """
        Calculate TTP using either Rule Of Thumb (ROT) or Maximum Tolerated Dose (MTD) strategies
        """
        
        if method == 'ROT':
            self.Simulate_AT(atThreshold=threshold, intervalLength=updating_interval, t_end=5000)
            self.Trim(dt=1)
            return self.ttp
        elif method == 'MTD':
            self.Simulate([[0, 4000, 1]])
            return max(self.resultsDf[self.resultsDf.TumourSize < 1.2 * self.resultsDf.TumourSize[0]].Time)
        else:
            raise ValueError("Unknown treatment protocol %s - should be either \'ROT\' or \'MTD\'" % method)
